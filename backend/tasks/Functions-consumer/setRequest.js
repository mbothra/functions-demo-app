const {
  ...
  task(
    ...
    .setAction(async (taskArgs) => {
      await setRequest(taskArgs.contract, taskArgs)
    })
  
  const setRequest = async (contract, taskArgs) => {
    ...
    const consumerContract = await consumerContractFactory.attach(contract)
  
    const requestConfig = require('../../api-request/Functions-Poke-api-config.js')
    ...
    // const functionsRequestSource = buildRequestCBOR({
    ...
    const setRequestTx = await consumerContract.setSource(
